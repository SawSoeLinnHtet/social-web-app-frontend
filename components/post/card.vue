...
<script setup>
    ...
    import { ChatBubbleOvalLeftIcon, PaperAirplaneIcon } from '@heroicons/vue/24/outline';
    ...
    const reactionPost = async (id) => {
        const likedPost = await reactPost(id);
        
        emit('reacted', likedPost, id);
    }

    const commentedPost = async (id) => {
        const commentedPost = await commentPost(id, { content: comment.value });
