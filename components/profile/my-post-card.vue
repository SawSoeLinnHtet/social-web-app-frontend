<template>
    <div class="bg-white rounded-xl border-1 border-gray-100 shadow-md p-5">
        <!-- Post header -->
            ...
            <button class="mb-2 text-red-500 cursor-pointer">
                <TrashIcon @click="deletePost(post.post_info.id)" class="size-5" />
            </button>
            </div>
        <!-- Post content -->
        ...
        <!-- Comment -->
        <div class="mt-5" :class="post?.comments?.length > 5 ? 'h-[200px] overflow-y-scroll' : ''">
...
<script setup>
    ...
    const emit = defineEmits(['deleted', 'reacted', 'commented']);

    const { commentPost, reactPost } = useProfile();
